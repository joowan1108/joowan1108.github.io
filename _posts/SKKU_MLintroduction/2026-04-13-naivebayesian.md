...
sunny, mild, high, strong일 때, yes였던 경우의 수 P(yes $\mid$ sunny, mild, high, strong)와 sunny, mild, high, strong일 때, no였던 경우의 수 P(no $\mid$ sunny, mild, high, strong)을 비교하면 된다.
...
Naive Bayesian에서 그럼 P(yes $\mid$ sunny, mild, high, strong)와 P(no $\mid$ sunny, mild, high, strong)는 어떻게 구하는 것일까?
...
이때, attribute 값이 실수인 domain일 때는 어떻게 될까? 예를 들어 A,B,C이면서 온도가 36.7도일 사건은 엄청 희귀하기 때문에 P(A,B,C,temp = 36.7 $\mid$ Yes) 값이 계속 0이 될 수가 있다. 분자의 한 확률값이 0이 된다면, 정당한 예측을 할 수 없게 된다.
...
P(SW = t, WW = k $\mid$ yes)을 구하기 위해서는

1. P(SW $\mid$ Yes)와 P(WW $\mid$ Yes), P(SW $\mid$ No)와 P(WW $\mid$ No)의 estimation을 구한 다음
...
**그런데 만약에 data가 부족해서 p(A $\mid$ yes) = 0 또는 P(A $\mid$ no) = 0이 되면 어떻게 될까?**

Data 부족으로 인해 yes였던 경우의 수 P(yes $\mid$ sunny, mild, high, strong)와 sunny, mild, high, strong일 때와 no였던 경우의 수 P(no $\mid$ sunny, mild, high, strong)을 정당하게 비교할 수 없게 된다.
