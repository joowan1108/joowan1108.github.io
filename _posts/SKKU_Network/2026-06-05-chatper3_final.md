...
예를 들어, receiver로부터 ACK가 올 때만 left wall을 오른쪽으로 움직이게 한다 (다음에 보내야 하는 데이터만 찾아내도록). 이런 경우에 right wall은 움직이지 않는다. 왜냐하면 아직 receiver의 process가 data을 다 읽지 않았기 때문에 데이터를 더 보내면 안되기 때문이다. 그래서 결국 다음 식이 만족한다. (new rwnd가 ackNO가 늘어난만큼 줄어듦)
...
TCP는 더 효과적인 flow control을 위해 send window의 크기를 더 세밀하게 조절하기 위해서 cwnd 값까지 활용해서 send window 크기를 결정한다. 
