...
itle: ELECTRA Pre-training text encoers as discriminators rather than generators  
# Background  
...
이미지 twostage  
**Two-Stage Training**  
Generator만 먼저 $\mathcal{L}_{MLM}$으로 n step동안 학습한 다음에 generator의 weights로 discriminator의 weights를 초기화하여 discriminator를 $\mathcal{L}_{Disc}$로 학습하는 방법을 실험했다. 이때 이 방법은 generator와 discriminator의 크기를 동일하게 해야 한다는 단점이 있다.  
