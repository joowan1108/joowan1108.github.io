...
$$
R_q(x_q,0) = \left\vert \left\vert q 
ight\vert 
ight\vert 
$$  
  
$$
R_k(x_k, 0) = \left\vert \left\vert k 
ight\vert 
ight\vert 
$$  
  
$$
R_g (x_q, x_k, 0) = \left\vert \left\vert q 
ight\vert 
ight\vert  \cdot \left\vert \left\vert k 
ight\vert 
ight\vert 
$$  
...
$$
R_g (x_q, x_k, 0) = R_q(x_q,m) \cdot R_k(x_k,m) = \left\vert \left\vert q 
ight\vert 
ight\vert  \cdot \left\vert \left\vert k 
ight\vert 
ight\vert 
...
이 관계식을 만족하는 제일 직관적인 풀이는 모든 m에 대해서 $R_q(x_q,m) = \left\vert \left\vert q 
ight\vert 
ight\vert $와 $R_q(x_q,m) = \left\vert \left\vert k 
ight\vert 
ight\vert $가 만족할 때이다. 이를 통해 함수 $R$은 위치값 m과 독립적임을 알 수 있다.  
...
$$  
f_q(x_q,m) = \left\vert \left\vert q 
ight\vert 
ight\vert  \cdot e^{i(\theta_q + m\theta + \gamma)}  
$$  
  
$$  
f_q(x_q,m) = (\left\vert \left\vert q 
ight\vert 
ight\vert  e^{i(\theta_q)}) \cdot e^{i(m\theta + \gamma)}  
$$  
...
ight\vert  Qx 
ight\vert 
ight\vert ^2 = (Qx)^T(Qx) = x^TQ^TQx = x^T I x = 
ight\vert 
ight\vert  x 
ight\vert 
ight\vert ^2  
...
이 특징 때문에 layer를 여러개 거쳐가도 크기가 유지되어 exploding이나 vanishing value 문제가 발생하지 않는다.  
...
Attention score 구하는 식을 content term $h_i = q_{[2i:2i+1]} k_{[2i:2i+1]}^*$과 position term $e^{i(m-n)\theta_i}$으로 나눌 수 있다. 이 계산을 쉽게 하기 위해 **Abel Transformation**을 적용할 수 있다.  
...
$$
\left\vert \text{Re} \left[ \sum_{i=0}^{d/2-1} q_{[2i:2i+1]} k_{[2i:2i+1]}^* e^{i(m-n)\theta_i} \right] \right\vert = \left\vert \sum_{i=0}^{\frac{d} {2}-1} (h_{i+1} - h_i) S_{i+1} \right\vert 
$$

$$
\left\vert \sum_{i=0}^{\frac{d} {2}-1} (h_{i+1} - h_i) S_{i+1} 
ight\vert  \le \sum_{i=0}^{\frac{d} {2}-1} \left\vert (h_{i+1} - h_i)
ight\vert  \left\vert S_{i+1}
ight\vert 
$$

$$
\sum_{i=0}^{\frac{d} {2}-1} \left\vert (h_{i+1} - h_i)
ight\vert  \left\vert S_{i+1}
ight\vert  \le (max \left\vert(h_{i+1} - h_i)
ight\vert ) \cdot \sum_{i=0}^{\frac{d} {2}-1} \left\vert S_{i+1}
ight\vert 
$$

이때 $(max \left\vert (h_{i+1} - h_i)
ight\vert )$은 relative distance를 결정하는 m과 n에 독립적이므로 $\sum_{i=0}^{\frac{d} {2}-1} \left\vert S_{i+1}
ight\vert $의 값만 관찰하면 된다. $\theta_i = 10000^{-\frac{2(i-1)}{d}}$일 때,  relative distance m-n이 증가할수록 $\sum_{i=0}^{\frac{d} {2}-1} \left\vert S_{i+1}
ight\vert $의 값은 decay한다.
