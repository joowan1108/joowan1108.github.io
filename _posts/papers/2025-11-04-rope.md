...
$$
R_g (x_q, x_k, 0) = || q || \cdot || k ||
$$  
  
m이 0이 아닐 때는 다음을 만족한다.  

$$
R_g (x_q, x_k, 0) = R_q(x_q,m) \cdot R_k(x_k,m) = || q || \cdot || k ||
$$  
  
이 관계식을 만족하는 제일 직관적인 풀이는 모든 m에 대해서 $R_q(x_q,m) = || q ||$와 $R_q(x_q,m) = ||k||$가 만족할 때이다. 이를 통해 함수 $R$은 위치값 m과 독립적임을 알 수 있다.  
...
$$  
f_q(x_q,m) = || q || \cdot e^{i(\theta_q + m\theta + \gamma)}  
$$  
  
$$  
f_q(x_q,m) = (|| q || e^{i(\theta_q)}) \cdot e^{i(m\theta + \gamma)}  
$$  
...
$$  
|| Qx ||^2 = (Qx)^T(Qx) = x^TQ^TQx = x^T I x = || x ||^2  
$$  
...
$$
| \text{Re} \left[ \sum_{i=0}^{d/2-1} q_{[2i:2i+1]} k^*_{[2i:2i+1]} e^{i(m-n)\theta_i} \right]  | = | \sum_{i=0}^{\frac{d} {2}-1} (h_{i+1} - h_i) S_{i+1} | 
$$

$$
| \sum_{i=0}^{\frac{d} {2}-1} (h_{i+1} - h_i) S_{i+1} | \le \sum_{i=0}^{\frac{d} {2}-1} |(h_{i+1} - h_i)| |S_{i+1}|
$$

$$
\sum_{i=0}^{\frac{d} {2}-1} |(h_{i+1} - h_i)| |S_{i+1}| \le (max |(h_{i+1} - h_i)|) \cdot \sum_{i=0}^{\frac{d} {2}-1} |S_{i+1}|
$$

이때 $(max |(h_{i+1} - h_i)|)$은 relative distance를 결정하는 m과 n에 독립적이므로 $\sum_{i=0}^{\frac{d} {2}-1} |S_{i+1}|$의 값만 관찰하면 된다. $\theta_i = 10000^{-\frac{2(i-1)}{d}}$일 때,  relative distance m-n이 증가할수록 $\sum_{i=0}^{\frac{d} {2}-1} |S_{i+1}|$의 값은 decay한다.
