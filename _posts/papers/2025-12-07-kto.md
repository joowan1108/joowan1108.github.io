...
> 보상 $r^{\ast}$을 최대화하는 optimal policy $\pi^{\ast}$은 다음 관계식을 만족한다.

> $$  
\pi^{\ast} (y \mid x) = \frac {1} {Z(x)} \pi_{ref} (y \mid x) \cdot exp(\frac {1} {\beta} r^{\ast}(x,y))  
$$

>양변에 log를 씌우고 정리하면

>$$  
log \frac {\pi^{\ast} (y \mid x) } {\pi_{ref} (y \mid x) } = \frac {1} {\beta} r^{\ast}(x,y) - log Z(x)  
$$  
  
>$\beta$를 양변에 곱하면
>$$  
\beta log \frac {\pi^{\ast} (y \mid x) } {\pi_{ref} (y \mid x) } = r^{\ast}(x,y) - \beta log Z(x)  
$$  
  
> 이때 HALO의 normalizing factor $\mathcal{l}$의 값이 $\beta$라면  
  
>$$  
r_{\theta}(x,y) = r^{\ast}(x,y) - \beta log Z(x)  
$$  
  
>$r_{\theta}$와 $r^{\ast}$는 동일한 equivalence class에 속하므로 동일한 optimal policy를 도출한다.  
...
**Theorem: Assuming the value function is logistic, for a reward function $r^{\ast}_a$ that maximizes (2), there exists a reward function in its equivalence class (i.e.,$r^{\ast}_b$ (x, y) = $r^{\ast}_a$ (x, y) + h(x) for some h(x)) that induces the same optimal policy $\pi^{\ast}$ and the same Bradley-Terry preference distribution but a different human value distribution.**  
...
두 reward function $r_{b}^{\ast}, r_{a}^{\ast}$가 동일한 equivalence class여서 $r_{b}^{\ast}(x,y) = r_{a}^{\ast}(x,y) + h(x)$을 만족한다고 하자. 이 둘이 동일한 optimal policy와 preference distribution을 도출하지만 인간이 느끼는 가치의 distribution는 다르다는 것을 통해 theorem을 증명한다.  
...
$$  
p_{r_b^{\ast}}(\gamma \mid y_1, .. y_K, x) = \prod_{k=1}^{K} \frac {\exp(r_b^{\ast}(x, y_{\gamma (k)}))} {\sum_{j=k}^{K} \exp(r_b^{\ast}(x, y_{\gamma(j)}))}  
$$  
  
여기서 $r_a^{\ast}(x,y) + h(x) = r_b^{\ast}(x,y)$을 대입하면  
  
$$p_{r_b^{\ast}}(\gamma \mid y_1, .. y_K, x) = \prod_{k=1}^{K} \frac {\exp(r_a^{\ast}(x, y_{\gamma (k)}) + h(x))} {\sum_{j=k}^{K} \exp(r_a^{\ast}(x, y_{\gamma(j)}) + h(x))}$$  
  
지수 법칙에 의해 항을 분리하면  
  
$$p_{r_b^{\ast}}(\gamma \mid y_1, .. y_K, x) = \prod_{k=1}^{K} \frac {\exp(h(x)) \cdot \exp(r_a^{\ast}(x, y_{\gamma (k)}))} {\sum_{j=k}^{K} \exp(h(x)) \cdot \exp(r_a^{\ast}(x, y_{\gamma(j)}))}$$  
  
$h(x)$는 합의 인덱스 $j$와 무관하므로 밖으로 묶어내어 분모/분자 소거가 가능함  
  
$$  
p_{r_b^{\ast}}(\gamma \mid y_1, .. y_K, x) = \prod_{k=1}^{K} \frac {\exp(r_a^{\ast}(x, y_{\gamma (k)}))} {\sum_{j=k}^{K} \exp(r_a^{\ast}(x, y_{\gamma(j)}))} = p_{r_a^{\ast}}(\gamma \mid y_1, .. y_K, x)  
$$  
  
따라서, reward function $r_a^{\ast}$와 $r_b^{\ast}$ 둘 다 동일한 preference distribution을 도출한다는 것이 증명된다.  
  
다음으로 reward function $r_a^{\ast}$와 $r_b^{\ast}$ 둘 다 동일한 optimal policy를 도출한다는 것을 증명한다.  
  
$$  
\begin{align*}  
\pi^{\ast}_{r_a}(y \mid x) &= \frac{1}{Z(x)} \pi_{\text{ref}}(y \mid x) \exp \left( \frac{1}{\beta} r^{\ast}_a(x, y) \right) \\  
&= \frac{\pi_{\text{ref}}(y \mid x) \exp \left( \frac{1}{\beta} r^{\ast}_a(x, y) \right) \exp \left( \frac{1}{\beta} h(x) \right)}{\sum_{y} \pi_{\text{ref}}(y \mid x) \exp \left( \frac{1}{\beta} r^{\ast}_a(x, y) \right) \exp \left( \frac{1}{\beta} h(x) \right)} \\  
&= \frac{\pi_{\text{ref}}(y \mid x) \exp \left( \frac{1}{\beta} (r^{\ast}_a(x, y) + h(x)) \right)}{\sum_{y} \pi_{\text{ref}}(y \mid x) \exp \left( \frac{1}{\beta} (r^{\ast}_a(x, y) + h(x)) \right)} \\  
&= \pi^{\ast}_{r_b}(y \mid x)  
\end{align*}  
$$  
  
따라서, 동일한 equivalence에 속하는 reward function $r_a^{\ast}$와 $r_b^{\ast}$ 둘 다 동일한 optimal policy를 도출한다는 것을 증명하였다.  
...
0에서의 $r^{\ast}_a(x,y)$의 human value의 taylor series는 다음과 같다.  
$$  
\sigma(0) + \sigma'(0)(r^{\ast}_a(x, y) - z_0) + \frac{\sigma''(0)}{2}(r^{\ast}_a(x, y) - z_0)^2 + \dots  
$$  
  
반대로 0에서의 $r^{\ast}_b(x,y) = r^{\ast}_a(x,y) + h(x)$의 human value의 taylor series는 다음과 같다.  
  
$$  
\sigma(h(x)) + \sigma'(h(x))(r^{\ast}_a(x, y) - z_0) + \frac{\sigma''(h(x))}{2}(r^{\ast}_a(x, y) - z_0)^2 + \dots  
$$  
  
**$\sigma$는 단조 증가 함수이기 때문에 이 두 taylor series가 같기 위해서는 h(x)=0을 만족해야한다. 그렇지 않다면 $r^{\ast}_b(x,y)$와 $r^{\ast}_a(x,y)$의 값은 다르다. 따라서, 동일한 equivalence class에 있다고 동일한 human value distribution을 도출하는 것이 아니다.**  
...
이때 $u = \beta (r_{\theta^{\ast}} (x,y_a) - r_{\theta^{\ast}} (x,y_b))$이므로  
  
$$  
\beta (r_{\theta^{\ast}} (x,y_a)) = \sigma^{-1}(p) + \beta(r_{\theta^{\ast}} (x,y_b))  
$$  
  
$\sigma^{-1}(x) = log \frac {x} {1-x}$이므로  
  
$$  
\beta \log \frac{\pi^{\ast}_\theta(y_a \mid x)}{\pi_{\text{ref}}(y_a \mid x)} = log \frac {p} {1-p} + \beta \log \frac{\pi^{\ast}_\theta(y_b \mid x)}{\pi_{\text{ref}}(y_b \mid x)}  
$$  
  
log을 다 벗기면  
  
$$  
\pi^{\ast}_\theta(y_a \mid x) = \underbrace {(\frac {p} {1-p})^{1/\beta} \cdot \frac {\pi_{\text{ref}}(y_a \mid x)} {\pi_{\text{ref}}(y_b \mid x)}}_{\text{t}} \cdot \pi^{\ast}_\theta(y_b \mid x)  
$$  
  
이때 t 값이 1보다 작다면, 즉 p가 너무 낮고 reference model이 제대로 align되어있지 않다면, 선호되는 output을 출력할 확률 $\pi_{\theta}^{\ast} (y_a \mid x)$ < 비선호되는 output을 출력할 확률 $\pi_{\theta}^{\ast} (y_b \mid x)$을 만족하게 된다.  
  
**따라서, DPO의 optimal policy는 상황에 따라 비선호되는 output $y_b$를 출력하는 policy가 될 수 있다.**  
  
**KTO의 경우**  
  
$u_a = \beta(r_{\theta}(x, y_a) - \mathbb{E_Q}[r_{\theta}(x, y')])$고 $u_b = \beta(r_{\theta}(x, y_b) - \mathbb{E_Q}[r_{\theta}(x, y')])$라고 할 때 $\sigma$의 성질 $1 - \sigma(-u) = \sigma(u)$을 통해 x에 대한 KTO의 loss를 다음처럼 바꿀 수 있다.  
...
**따라서 KTO의 policy가 loss를 최소화하기 위해서는**
1) $u_a$를 최대화하기 위해 $\pi_{\theta}(y_a \mid x)$는 1로 수렴해야 하고
2) $u_b$를 최소화하기 위해 $\pi_{\theta}(y_b|x)$가 $0$으로 가야 한다.
확률의 합 $\pi_{\theta}(y_a \mid x) + \pi_{\theta}(y_b \mid x)$은 1이어야 하므로, **이를 만족하는 유일한 policy는 $\pi_\theta(y_a|x) = 1$, $\pi_\theta(y_b|x) = 0$을 만족하는 policy이다.**

즉, 답변의 label을 판단할 확률이 조금만 좋아도 ($p>0.5$만 만족하면) KTO의 optimal policy $\pi^{\ast}_\theta(y \mid x)$는 항상 desirable output을 생성한다. 
  
$$
\pi^{\ast}_{\theta}(y|x) = \mathbb{1}[y = y_a]
$$
