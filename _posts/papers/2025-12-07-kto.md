...
**Theorem: Assuming the value function is logistic, for a reward function $r^{*}_a$ that maximizes (2), there exists a reward function in its equivalence class (i.e.,$r^{*}_b$ (x, y) = $r^{*}_a$ (x, y) + h(x) for some h(x)) that induces the same optimal policy $\pi^{*}$ and the same Bradley-Terry preference distribution but a different human value distribution.**  
