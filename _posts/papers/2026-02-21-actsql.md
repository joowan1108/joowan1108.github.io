...
1. **Table(Column)**: 각 table 명칭과 column 이름들을 나열  
2. **Table(Column)(PF)**: 각 table 명칭과 column 이름들과 각 table 끝에 Primary key와 foreign key들까지 전달  
3. **Create(NoPF)**: Table과 column 명들을 SQL 문법으로 전달  
4. **Create(EoC)**: Create(NoPF)에 이어서 각 column 끝에 primary key인지 foreign key인지 정보 전달  
5. **Create(EoT)**: Create(NoPF)에서 각 table 끝에 primary key와 foreign key 정보 추가  
  
![joowan1108]({{site.url}}/images/papers/actsql/appendixc1.PNG) 
