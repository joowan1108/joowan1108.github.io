...
* Compute $d = \mathbb{\hat E_t} \left[ \text{KL} \left[ \pi_{\theta_{old}} (\cdot \mid s_t), \pi_{\theta} (\cdot \mid s_t) \right] \right]$  
...
이때, $c_1, c_2$는 상수이고 S는 entropy bonus로 $s_t$에서 다양한 시도를 할 수 있게 해주는 항이다. $L_t^{VF}(\theta)$은 $(V_{theta}(s_t) - V^{targ}_t)^2$으로 실제 value 값과 value function이 예측한 값의 squared-error loss이다.  
