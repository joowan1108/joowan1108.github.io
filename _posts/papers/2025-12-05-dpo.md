...
이 $p^{*}$ 로부터 각 $x^{(i)}$ 에 대한 두 답안 $y_w^{(i)}, y_l^{(i)}$ pair로 이루어진 dataset $D$ 을 sampling하여 이 dataset을 바탕으로 reward model $r_{\phi}(x,y)$ 을 다음 objective으로 학습시킬 수 있다. 이렇게 얻은 reward function은 어떤 답안이 사람에게 더 선호될 지 맞출 수 있는 함수가 된다.
...
$$  
max_{\pi_{\theta}} \mathbb{E}_{x \sim D, y \sim \pi_{\theta} (y \mid x)} \left[ r_\phi(x, y) \right] - \beta \cdot D_{KL} \left[ \pi_{\theta}(y \mid x) \mid \mid {\pi^{\text{ref}}(y \mid x)} \right] \,\, (c)  
$$  
...
이때, Z(x)가 $\sum_{y} (\pi_{\text{ref}}(y \mid x) \times exp( \frac {1} {\beta} \cdot r_{\theta}(x,y)))$라고 한다면, $\pi_{\text {ref}}$에만 의존하는 확률 분포 $\pi^*(y \mid x)$가 나온다.  
  
$$  
\pi^*(y \mid x) = \frac {\pi_{\text{ref}}(y \mid x) \times exp( \frac {1} {\beta} \cdot r_{\theta}(x,y))} {Z(x)}  (7)  
$$  
  
>모든 y에 대해 $\pi^* (y \mid x) \ge 0$ 이면서 $\sum_{y} \pi^*(y \mid x) = 1$ 이기 때문에 valid한 확률 분포이다.
    
Z(x)는 y에 대한 함수가 아니기 때문에 equation (7)을 대입하면 objective가 다음과 같이 변한다.  
...
이때 Z(x)는 $\phi$와 독립적이기 때문에 고려하지 않아도 된다. Optimal policy를 구하기 위해서는 이 objective을 최소화하는 ($D_{KL}$값을 최소화하는) $\phi$를 찾아야 한다. Gibb's equality에 따르면, $D_{KL}$값이 최소일 때는 두 분포 $\pi_{\phi} (y \mid x)$ 와 $\pi^* (y \mid x)$ 가 동일할 때이다. 따라서, KL-constraint을 지닌 reward maximization objective을 최대화하는 policy $\pi_{\phi}$ 는 $\pi^*$ 인 것이다.  
...
>$$
r(x,y) \xrightarrow{f} \beta \log \frac{\pi(y \mid x)}{\pi_{\text{ref}}(y \mid x)}
$$
...
여기서 문제는 Theorem 1은 reward function을 $\beta \log \frac{\pi(y \mid x)}{\pi_{\text{ref}}(y \mid x)}$으로 대체를 할 수 있다는 것만 보여준다. *하지만 이렇게 대체하여 학습했을 때, policy가 올바른 human preference을 학습했는지 알 수 없다*. **따라서 reward function의 reparameterization을 사용하여 학습했을 때 도출된 optimal policy가 의도된 human preference을 잘 ranking하는 reward function과 일대일 대응이 되는가를 증명해야 한다.**

본 논문은 다음 명제 제시하고 증명한다.
**Proposition 1**
...
**하지만 Theorem 1과 Proposition 1에 따르면 Plackett-Luce 기반의 모든 reward function은 $\beta \log \frac{\pi(y \mid x)} {\pi_{\text{ref}}(y \mid x)}$ 으로 대체할 수 있으며, 대체하고 학습을 한 optimal policy는 필연적으로 올바른 human preference를 가진 policy라는 것이다. 따라서,  RLHF와 같은 과정을 거치지 않아도 된다는 것이다. 그래서 DPO의 objective는 다음과 같이 reward function을 필요로 하지 않는다.**
