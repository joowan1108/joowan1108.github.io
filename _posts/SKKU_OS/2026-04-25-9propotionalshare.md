...
그렇지 않다. Stride scheduling에서 새로운 process가 들어온다고 해보자. 그래서 pass의 값을 0으로 했다고 하자. 그런데 예를 들어서 진행이 오래 되었어서 다른 process들의 pass 값이 매우 크다고 해보자. 그렇다면 새로운 process의 pass 값은 한동안 제일 낮을 것이므로 CPU 자원을 독차지하게 된다.
