...
Address space가 16KB이고 page 크기는 64 byte이라고 하자.

그러면 총 page 수는 $16*2^10 / 2^6 = 256$ 이고 그럼 VPN은 $256 = 2^8$ 이므로 8개의 bit으로 되어있으며 offset은 $64 = 2^6$ 이므로 6개의 bit으로 되어있다.

이로부터 two level page table을 만든다고 할 때, 우선 linear table을 page 크기로 나눠야한다. PTE의 크기가 4 byte라고 할 때, linear page table의 크기는 $256 * 4 = 1024$ byte이므로 1KB이다. Linear page table은 page 크기로 나누었을 때, 16개의 부분으로 나뉘어지고 각 부분에 16개의 PTE가 들어간다( page 크기 64 / pte 크기 4 = 16). 

Page directory에서 중요한 것은 page directory index을 VPN으로부터 얻고 또 VPN으로 page table 안에 access하는 것이다. Linear page table은 16개의 부분으로 이루어지므로 page directory index는 4 bit이다. 

![joowan1108]({{site.url}}/images/SKKU_OS/pg249.png)

Page directory 내에서 접근하고 싶은 PDE의 주소는 다음 식으로 계산한다.

$$
\text{PDEAddr = PageDirBase + (PDIndex * sizeof (PDE))}
$$

이 PDE가 valid해서 이 주소로 갈 수 있다고 해보자. 이 주소 안에는 접근하려는 virtual address space에 대응하는 physical address의 PFN을 얻을 수 있다. Page directory을 통해 이동한 page 크기의 table에서 원하는 PFN을 얻기 위해서 VPN의 나머지 bit (page table index)을 사용한다.

![joowan1108]({{site.url}}/images/SKKU_OS/pg250.png)
...
실제 two level page table을 봐보자

![joowan1108]({{site.url}}/images/SKKU_OS/pg250_1.png)

Page directory는 valid한 PFN에 대한 page table 주소만 들고 있는 것을 볼 수 있다. Page directory에서 100 PFN으로 갔다고 하자. 그럼 중간의 page으로 들어가게 된다.

이 page table을 통해 PFN:101 page의 PFN 55의 virtual address을 얻어보자.

우선 PFN:101 page으로 가기 위해서는 Page directory의 index가 1111이므로 virtual address는 $11 11xx xxxx xxxx_2$ 가 된다. PFN: 101 page으로 들어갔을 때 PFN 55의 index는 1110이기에 offset이 0이라고 할 때 PTE의 virtual address는 $11 1111 1000 0000_2$ 이다. 

그럼 PFN 55, offset 0에 접근한다고 했을 때 실제로 연결되는 physical memory 상의 주소를 얻어보자.

우선 offset은 6 bit이고 0으로 둘 것이므로 Physical address = $0011 0111_2$ (55) << 6 + offset = $00 1101 1100 0000_2$ 이다. 

## Inverted Page Table

Inverted page table은 반대로 physical page마다 page table을 갖게 하는 구조이다. 즉, index로 physical frame number가 사용되고 각 frame number에 대응되는 VPN이 존재하는 것이다. Physical frame number 수에 비례하여 page table 크기가 정해지기에 메모리를 많이 아낄 수 있지만, (process마다 page table을 갖는 것보단..) VPN -> PFN address translation을 할 때 일일이 대응시키면서 봐야하기 때문에 속도가 느리다.
