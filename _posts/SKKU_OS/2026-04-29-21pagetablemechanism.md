...
Page replacement policy을 적용하기 위해서는 present bit와 valid bit 정보를 사용한다. 
...
### Replacement가 실제로 이어진다면?

Replacement가 일어날 때는 physical memory가 꽉 차서 그런것이라고 가정을 했었다. 하짐나 실제로는 physical memory가 가득 찰 때까지 기다리지 않는다. 
